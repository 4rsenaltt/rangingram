...
public class ContactsAdapter extends BaseSectionsAdapter {
    ...
    public View getItemView(int section, int position, View convertView, ViewGroup parent) {
        ...
        if (type == 4) {
            ...
            if (needPhonebook) {
                actionCell.setTextAndIcon(LocaleController.getString("InviteFriends", R.string.InviteFriends), theme.ContactsAdapter_actionCell_invite_R);
            } else if (isAdmin) {
                actionCell.setTextAndIcon(LocaleController.getString("InviteToGroupByLink", R.string.InviteToGroupByLink), theme.ContactsAdapter_actionCell_inviteg_R);
            } else {
                if (position == 0) {
                    actionCell.setTextAndIcon(LocaleController.getString("NewGroup", R.string.NewGroup), theme.ContactsAdapter_actionCell_newG_R);
                } else if (position == 1) {
                    actionCell.setTextAndIcon(LocaleController.getString("NewSecretChat", R.string.NewSecretChat), theme.ContactsAdapter_actionCell_neschat_R);
                } else if (position == 2) {
                    actionCell.setTextAndIcon(LocaleController.getString("NewChannel", R.string.NewChannel), theme.ContactsAdapter_actionCell_newchannel_R);
                }
            }
        } else if (type == 1) {
            if (convertView == null) {
